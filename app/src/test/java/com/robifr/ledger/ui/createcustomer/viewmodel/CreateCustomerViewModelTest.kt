...
open class CreateCustomerViewModelTest {
  protected open lateinit var _viewModel: CreateCustomerViewModel
  protected var _customerRepository: CustomerRepository = mock()

  @BeforeEach
  open fun beforeEach() {
    reset(_customerRepository)
    _viewModel = CreateCustomerViewModel(_customerRepository)
  }
  ...
  open fun `on save`(createdCustomerId: Long) = runTest {
    val customer: CustomerModel =
        CustomerModel(id = null, name = "Amy", balance = 100L, debt = (-100).toBigDecimal())
    _viewModel.onNameTextChanged(customer.name)
    _viewModel.onBalanceChanged(customer.balance)
    _viewModel.onDebtChanged(customer.debt)

    whenever(_customerRepository.add(customer))
