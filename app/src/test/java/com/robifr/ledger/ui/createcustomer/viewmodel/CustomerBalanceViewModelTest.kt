...
open class CustomerBalanceViewModelTest {
  ...
  @ParameterizedTest
  @ValueSource(longs = [0L, 100L, 500L])
  fun `on withdraw amount text changed`(currentBalance: Long) {
    ...
    val amountToReduce: Long = 500L
    val formattedAmountToReduce: String = CurrencyFormat.format(amountToReduce.toBigDecimal(), "")
    val balanceAfter: BigDecimal =
        _viewModel.uiState.safeValue.balance.toBigDecimal().subtract(amountToReduce.toBigDecimal())
    ...
    assertEquals(
        ...
        _viewModel.balanceView.withdrawBalanceState.safeValue,
        "Revert the state if the withdrawn balance is insufficient")
  }
