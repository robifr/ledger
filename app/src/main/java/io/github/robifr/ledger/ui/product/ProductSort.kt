...
import io.github.robifr.ledger.databinding.ProductDialogSortByBinding

class ProductSort(private val _fragment: ProductFragment) {
  private val _dialogBinding: ProductDialogSortByBinding =
      ProductDialogSortByBinding.inflate(_fragment.layoutInflater).apply {
        for (sortBy in ProductSortMethod.SortBy.entries) {
          ...
          radioGroup.findViewWithTag<RadioButton>(sortBy.toString())?.setOnClickListener {
            _fragment.productViewModel.onSortMethodChanged(sortBy)
            _fragment.productViewModel.onSortMethodDialogClosed()
          }
        }
      }
  private val _dialog: BottomSheetDialog =
      BottomSheetDialog(_fragment.requireContext(), R.style.BottomSheetDialog).apply {
        ...
        setOnDismissListener { _fragment.productViewModel.onSortMethodDialogClosed() }
      }

  init {
    _fragment.fragmentBinding.sortByChip.setText(R.string.product_sortBy)
    _fragment.fragmentBinding.sortByChip.setOnClickListener {
      _fragment.productViewModel.onSortMethodDialogShown()
    }
  }

  fun showDialog(selectedSortMethod: ProductSortMethod) {
