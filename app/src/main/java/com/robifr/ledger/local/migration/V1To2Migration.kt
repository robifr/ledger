 ...
 */
class V1To2Migration(private val _settingsPreferences: SettingsPreferences) : Migration(1, 2) {
  override fun migrate(database: SupportSQLiteDatabase) {
    val languageTagUsed: String =
        _settingsPreferences.sharedPreferences.getString(
            SettingsPreferences.KEY_LANGUAGE_USED, null) ?: LanguageOption.ENGLISH_US.languageTag
    ...
    try {
      database.execSQL(
          "UPDATE `customer` SET `balance` = `balance` * ?", arrayOf(fractionDigitsMultiplier))
      database.execSQL(
      ...
      while (cursor.moveToNext()) {
        ...
        val updatedDiscount: Long =
            cursor.getLong(cursor.getColumnIndexOrThrow("discount")) * fractionDigitsMultiplier
