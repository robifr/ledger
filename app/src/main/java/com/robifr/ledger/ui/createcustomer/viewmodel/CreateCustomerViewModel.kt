...
@Inject
constructor(protected val _customerRepository: CustomerRepository) : ViewModel() {
  protected val _balanceView: CustomerBalanceViewModel by lazy { CustomerBalanceViewModel(this) }
  val balanceView: CustomerBalanceViewModel
    get() = _balanceView
  ...
  open fun onSave() {
    ...
    _addCustomer(_inputtedCustomer())
  }

  /**
   * Get current inputted customer from any corresponding inputted live data. If any live data is
   * set using [MutableLiveData.postValue], calling this method may not immediately reflect the
   * latest changes. For accurate results in asynchronous operations, consider calling this method
   * inside [Observer].
   */
  internal open fun _inputtedCustomer(): CustomerModel =
      CustomerModel.toBuilder()
          .setName(_uiState.safeValue.name)
          .setBalance(_uiState.safeValue.balance)
          .setDebt(_uiState.safeValue.debt)
          .build()

  private fun _addCustomer(customer: CustomerModel) {
    _customerRepository.add(customer).thenAcceptAsync { id: Long? ->
