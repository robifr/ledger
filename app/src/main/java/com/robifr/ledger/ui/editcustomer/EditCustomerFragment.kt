...
class EditCustomerFragment : CreateCustomerFragment() {
  override val _createCustomerViewModel: EditCustomerViewModel by viewModels()

  override fun onViewCreated(view: View, savedInstance: Bundle?) {
    super.onViewCreated(view, savedInstance)
    fragmentBinding.toolbar.setTitle(R.string.editCustomer)
    _createCustomerViewModel.editResultState.observe(viewLifecycleOwner) {
      it.handleIfNotHandled(::_onEditResultState)
    }
  }

  private fun _onEditResultState(state: EditCustomerResultState) {
    state.editedCustomerId?.let { id ->
      parentFragmentManager.setFragmentResult(
          Request.EDIT_CUSTOMER.key(),
          Bundle().apply { putLong(Result.EDITED_CUSTOMER_ID_LONG.key(), id) })
    }
    finish()
  }
