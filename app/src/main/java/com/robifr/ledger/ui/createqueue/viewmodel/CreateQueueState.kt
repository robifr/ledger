...
data class CreateQueueState(
  ...
  val customerNameCropped: String
    get() =
        if (customer != null && customer.name.length > 12) customer.name.substring(0, 12)
        else customer?.name ?: ""

  @get:ColorRes
  val customerDebtColor: Int
    get() =
        if (customer != null && customer.debt.compareTo(0.toBigDecimal()) < 0) R.color.red
