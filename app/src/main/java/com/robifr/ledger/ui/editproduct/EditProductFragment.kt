...
class EditProductFragment : CreateProductFragment() {
  override val _createProductViewModel: EditProductViewModel by viewModels()

  override fun onViewCreated(view: View, savedInstance: Bundle?) {
    super.onViewCreated(view, savedInstance)
    fragmentBinding.toolbar.setTitle(R.string.editProduct)
    _createProductViewModel.editResultState.observe(viewLifecycleOwner) {
      it.handleIfNotHandled(::_onEditResultState)
    }
  }

  private fun _onEditResultState(state: EditProductResultState) {
    state.editedProductId?.let { id ->
      parentFragmentManager.setFragmentResult(
          Request.EDIT_PRODUCT.key(),
          Bundle().apply { putLong(Result.EDITED_PRODUCT_ID_LONG.key(), id) })
    }
    finish()
  }
