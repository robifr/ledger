...
open class CreateCustomerFragment : Fragment(), Toolbar.OnMenuItemClickListener {
  ...
  protected open val _createCustomerViewModel: CreateCustomerViewModel by viewModels()
  val createCustomerViewModel: CreateCustomerViewModel
    get() = _createCustomerViewModel
  ...
  override fun onViewCreated(view: View, savedInstance: Bundle?) {
    this._inputName = CreateCustomerName(this)
    this._inputBalance = CreateCustomerBalance(this)
    this._inputDebt = CreateCustomerDebt(this)
  ...
  override fun onMenuItemClick(item: MenuItem?): Boolean =
      when (item?.itemId) {
        R.id.save -> {
          _createCustomerViewModel.onSave()
          true
        }
        else -> false
      }
  ...
  private fun _onResultState(state: CreateCustomerResultState) {
    state.createdCustomerId?.let { id ->
      parentFragmentManager.setFragmentResult(
          Request.CREATE_CUSTOMER.key(),
          Bundle().apply { putLong(Result.CREATED_CUSTOMER_ID_LONG.key(), id) })
    }
    finish()
  }
  ...
  private fun _onUiState(state: CreateCustomerState) {
    ...
    _inputDebt.setInputtedDebt(state.debt)
  }
