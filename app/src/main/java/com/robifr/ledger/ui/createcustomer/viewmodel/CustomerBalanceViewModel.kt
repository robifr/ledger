...
class CustomerBalanceViewModel(private val _createCustomerViewModel: CreateCustomerViewModel) {
  ...
  fun onBalanceAmountTextChanged(amount: String) {
    var amountToAdd: BigDecimal = BigDecimal.ZERO
    try {
      amountToAdd =
  ...
  fun onShowWithdrawBalanceDialog() {
    _withdrawBalanceState.setValue(
        _withdrawBalanceState.safeValue.copy(
            isDialogShown = true,
            availableAmountToWithdraw = _createCustomerViewModel._inputtedCustomer().balance))
  }
  ...
  fun onWithdrawAmountTextChanged(amount: String) {
    var amountToWithdraw: BigDecimal = BigDecimal.ZERO
    try {
      amountToWithdraw =
          CurrencyFormat.parse(amount, AppCompatDelegate.getApplicationLocales().toLanguageTags())
    } catch (ignore: ParseException) {}
    ...
    val isLeftOverBalanceAvailable: Boolean = balanceAfter.compareTo(BigDecimal.ZERO) >= 0
    _withdrawBalanceState.setValue(
        _withdrawBalanceState.safeValue.copy(
            // Revert back when when there's no more available balance to withdraw.
            formattedAmount =
                if (isLeftOverBalanceAvailable) amount
   ...
   * is set using [MutableLiveData.postValue], calling this method may not immediately reflect the
   * latest changes. For accurate results in asynchronous operations, consider calling this method
   * inside [Observer].
   */
   ...
   * data is set using [MutableLiveData.postValue], calling this method may not immediately reflect
   * the latest changes. For accurate results in asynchronous operations, consider calling this
   * method inside [Observer].
   */
