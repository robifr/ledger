...
@Inject
constructor(protected val _productRepository: ProductRepository) : ViewModel() {
  ...
  open fun onSave() {
    ...
    _addProduct(_inputtedProduct())
  }

  /**
   * Get current inputted product from any corresponding inputted live data. If any live data is set
   * using [MutableLiveData.postValue], calling this method may not immediately reflect the latest
   * changes. For accurate results in asynchronous operations, consider calling this method inside
   * [Observer].
   */
  protected open fun _inputtedProduct(): ProductModel {
    var price: Long = 0L
    try {
      price =
    ...
    return ProductModel.toBuilder().setName(_uiState.safeValue.name).setPrice(price).build()
  }

  private fun _addProduct(product: ProductModel) {
    _productRepository.add(product).thenAcceptAsync { id: Long? ->
