...
class CreateProductName(private val _fragment: CreateProductFragment) {
  ...
  fun setInputtedNameText(name: String) {
    val currentText: String = _fragment.fragmentBinding.name.getText().toString()
    if (currentText == name) return
  ...
  fun setError(message: String?) {
    _fragment.fragmentBinding.nameLayout.setError(message)
  }
}
