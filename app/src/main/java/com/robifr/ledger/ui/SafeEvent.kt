 ...
 *    private val MutableLiveData<SafeEvent<String>> _state = MutableLiveData()
 *    val LiveData<SafeEvent<String>> state
 *      get() = _state
 * }
 *
...
class SafeEvent<T>(private val _value: T) {
  ...
  fun valueIfNotHandled(): T? {
    if (_isHandled) return null
    _isHandled = true
    return _value
  }

  companion object {
    fun <T> observeOnce(liveData: LiveData<T>, observer: Observer<T?>, filter: Predicate<T?>) =
