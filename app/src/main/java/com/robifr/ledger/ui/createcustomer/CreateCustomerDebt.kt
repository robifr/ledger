...
class CreateCustomerDebt(private val _fragment: CreateCustomerFragment) {
  fun setInputtedDebt(debt: BigDecimal) {
    _fragment.fragmentBinding.debt.setText(
        CurrencyFormat.format(debt, AppCompatDelegate.getApplicationLocales().toLanguageTags()))
    _fragment.fragmentBinding.debt.setTextColor(
        // Red for negative debt.
        if (debt.compareTo(BigDecimal.ZERO) < 0) _fragment.requireContext().getColor(R.color.red)
        else _fragment.requireContext().getColor(R.color.text_disabled))
  }
}
