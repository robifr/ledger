...
constructor(
  ...
  open fun inputtedQueue(): QueueModel =
      QueueModel(
    ...
    _customerRepository.selectById(customerId).thenAcceptAsync { customer: CustomerModel? ->
      if (customer == null) {
        _snackbarState.postValue(
            SafeEvent(SnackbarState(StringResource(R.string.createQueue_fetchCustomerError))))
      }
  ...
  fun selectProductById(productId: Long?): LiveData<ProductModel?> {
    ...
    _productRepository.selectById(productId).thenAcceptAsync { product: ProductModel? ->
      if (product == null) {
        _snackbarState.postValue(
            SafeEvent(SnackbarState(StringResource(R.string.createQueue_fetchProductError))))
      }
  ...
  private fun _addQueue(queue: QueueModel) {
    _queueRepository.add(queue).thenAcceptAsync { id: Long? ->
